...
class EventController extends AbstractController
{
		...
		$page = 1): Response
	{
		...
		if ($user) {
			$userId = $user->getId();
		// Lorsqu'un user affiche la page la 1re fois, la school par défaut est la sienne
		...
		$paginator = $eventRepository->findByFilters($value,
			 										 $start,
													 $end,
													 $school,
													 $page,
	...
	public function withdraw(Event $event, Withdraw $withdraw): Response
	{
		...
		$withdraw->leave();
		$this->addFlash(Alert::SUCCESS, Messages::SUCCESS_UNSUBSCRIBE . $event->getName());
